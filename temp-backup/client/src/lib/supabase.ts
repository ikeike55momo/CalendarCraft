...
export const signInWithGoogle = async () => {
  ...
  try {
    ...
    console.log('Google Client ID:', googleClientId ? `${googleClientId.substring(0, 8)}...` : '未設定');

    // Supabaseの標準OAuth機能を使用（シンプルな設定）
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { 
        redirectTo: redirectUrl,
        // スコープを指定しない（デフォルトのスコープを使用）
        // クエリパラメータも最小限に
        queryParams: {
          prompt: 'consent'
        }
