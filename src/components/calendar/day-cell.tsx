        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            <div className="space-y-1.5 relative z-10">
              ...
              {attendance && attendance.attendanceLog?.clockIn && (
                <div className="flex items-center gap-2 text-xs text-gray-600 bg-gradient-to-r from-gray-100/80 to-gray-50/80 p-1.5 rounded-lg">
                  ...
                  <span className="font-medium">
                    {format(new Date(attendance.attendanceLog.clockIn), "HH:mm")}
                  </span>
                </div>
              )}
          ...
          <div className="space-y-4">
            ...
            {attendance && attendance.attendanceLog && (
              <div className="space-y-2">
                ...
                {attendance.attendanceLog.clockIn && (
                ...
                {attendance.attendanceLog.clockOut && (
                  <div className="flex items-center gap-2 text-sm">
                    <Clock className="h-4 w-4 text-gray-400" />
                    <span className="font-medium">
                      {format(new Date(attendance.attendanceLog.clockOut), "HH:mm")}
                    </span>
                    <span className="text-gray-500">退勤</span>
                  </div>
                )}
                {attendance.attendanceLog.breaks?.map((breakTime, index) => (
                  <React.Fragment key={index}>
                    <div className="flex items-center gap-2 text-sm">
                      <Clock className="h-4 w-4 text-gray-400" />
                      <span className="font-medium">
                        {format(new Date(breakTime.start), "HH:mm")}
                      </span>
                    ...
                    {breakTime.end && (
                      <div className="flex items-center gap-2 text-sm">
                        <Clock className="h-4 w-4 text-gray-400" />
                        <span className="font-medium">
                          {format(new Date(breakTime.end), "HH:mm")}
                        </span>
